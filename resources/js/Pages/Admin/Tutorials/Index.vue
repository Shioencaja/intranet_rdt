...
const confirmDelete = async (id) => {
    ...
    if (result.isConfirmed) {
        Inertia.delete(route("admin.tutorial_destroy", id), {
            onError: (errors) => {
                console.log(errors)
                Swal.fire({
                    icon: "error",
...
const getSearch = throttle(() => {
    Inertia.get(route("admin.tutorial"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        <TableCustom add="admin.tutorial_add" class="mt-5">
            <template #search>
            ...
            <template #thead>
                ...
                <TheadTable>Linea</TheadTable>
                <TheadTable>Acciones</TheadTable>
            </template>
            <template #tbody>
                <Tbody v-for="(item, index) in tutorials.data" :key="index">
                    ...
                    <td
                        class="flex border-grey-light border hover:bg-gray-100 p-3"
                    >
                        <Link :href="route('admin.tutorial_edit', item.id)">
                            <Icons
                                type="edit"
