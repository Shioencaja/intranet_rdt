...
const getSearch = throttle(() => {
    Inertia.get(route("admin.coupons"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        ...
        <TableCustom add="admin.coupons_add" class="mt-5" titlePage="CUPONES">
            ...
            <template #tbody>
                <Tbody v-for="(item, index) in coupons.data" :key="index">
                    ...
                    <td
                        class="flex  p-3"
                    >
                        <Link :href="route('admin.coupons_edit', item.id)">
                            <Icons
                                type="edit"
