...
<template>
    <LayoutAdmin>
        <FormCard title="REGISTRO DE CUPONES" :submit="submit" back="admin.coupons">
            ...
            <div class="line h-32">
                <textarea class="line__input h-44" autocomplete="off" id="description" type="text"
                    v-model="form.description"></textarea>
                <span for="description" class="line__placeholder">Descripción</span>
            </div>

         
            <div class="line">
                <input class="line__input" autocomplete="off" type="file"
                    @input="form.image = $event.target.files[0]" />
                <small class="flex absolute inset-y-0 right-0 items-center pr-3">
                    (434 x 226)
                </small>
            </div>
            <small class="mt-5"> (434 x 226)</small>
            ...
            <div>
                <div class="flex items-center justify-center w-full">
                    <label for="dropzone-file"
                        ...
                        <input id="dropzone-file" type="file" class="hidden" @change="(e) => uploadFiles(e)"
                ...
                <template v-for="(item, index) in form.files" :key="index">
                    <div class="flex w-full shadow-sm py-2">
                        ...
                        <div @click="removeUploadFiles(index)" class="cursor-pointer">
                            <Icons type="delete" class="cursor-pointer text-red-400" />
                        </div>
                    </div>
                </template>
            </div>
