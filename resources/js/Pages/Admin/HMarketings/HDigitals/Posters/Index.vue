...
const getSearch = throttle(() => {
    Inertia.get(route("admin.poster"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        ...
        <TableCustom add="admin.poster_add" class="mt-5" titlePage="PÓSTER">
            ...
            <template #tbody>
                <Tbody v-for="(item, index) in marketings.data" :key="index">
                    ...
                    <td
                        class="flex  p-3"
                    >
                        <Link :href="route('admin.poster_edit', item.id)">
                            <Icons
                                type="edit"
