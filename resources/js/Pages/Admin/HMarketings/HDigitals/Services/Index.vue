...
const getSearch = throttle(() => {
    Inertia.get(route("admin.service"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        ...
        <TableCustom add="admin.service_add" class="mt-5" titlePage="MENÚ DE SERVICIO">
            ...
            <template #tbody>
                <Tbody v-for="(item, index) in marketings.data" :key="index">
                    ...
                    <td
                        class="flex  p-3"
                    >
                        <Link :href="route('admin.service_edit', item.id)">
                            <Icons
                                type="edit"
