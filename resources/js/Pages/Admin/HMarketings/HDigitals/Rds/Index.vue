...
const getSearch = throttle(() => {
    Inertia.get(route("admin.red"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        ...
        <TableCustom add="admin.red_add" class="mt-5" titlePage="IMÁGENES DE REDES SOCIALES">
            ...
            <template #tbody>
                <Tbody v-for="(item, index) in marketings.data" :key="index">
                    ...
                    <td
                        class="flex  p-3"
                    >
                        <Link :href="route('admin.red_edit', item.id)">
                            <Icons
                                type="edit"
