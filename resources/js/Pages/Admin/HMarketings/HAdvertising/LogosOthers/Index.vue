...
const getSearch = throttle(() => {
    Inertia.get(route("admin.other_logo"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        ...
        <TableCustom add="admin.other_logo_add" class="mt-5" titlePage="OTROS LOGOS">
            ...
            <template #tbody>
                <Tbody v-for="(item, index) in marketings.data" :key="index">
                    ...
                    <td
                        class="flex  p-3"
                    >
                        <Link :href="route('admin.other_logo_edit', item.id)">
                            <Icons
                                type="edit"
