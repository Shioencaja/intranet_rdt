...
const form = useForm({
    route:'admin.other_logo',
    title: props?.marketing?.title,
    description: props?.marketing?.description,
    image: null,
    files: [],
});
...
const destroyFile =async(id)=>{
    const resp = await axios.delete(route("admin.marketing_file_delete",id));
    if(resp.data.status){
...
const submit = () => {
    form.post(route("admin.marketing_update",props.marketing?.id), {});
};
</script>
...
<template>
    <LayoutAdmin>
        <FormCard
            title="EDITAR OTRO LOGO"
            :submit="submit"
            back="admin.other_logo"
        >
            ...
            <div class="my-5">
                <template v-for="(item, index) in filesOld" :key="index">
                    <div class="flex w-full shadow-sm py-2">
                        ...
                        <a :href="item.url_patch" target="_blank" rel="noopener noreferrer"><Icons
                                type="download"
