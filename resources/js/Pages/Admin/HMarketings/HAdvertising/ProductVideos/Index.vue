...
const getSearch = throttle(() => {
    Inertia.get(route("admin.product_v"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        ...
        <TableCustom add="admin.product_v_add" class="mt-5" titlePage="VIDEO DE PRODUCTO">
            ...
            <template #tbody>
                <Tbody v-for="(item, index) in marketings.data" :key="index">
                    ...
                    <td
                        class="flex  p-3"
                    >
                        <Link :href="route('admin.product_v_edit', item.id)">
                            <Icons
                                type="edit"
