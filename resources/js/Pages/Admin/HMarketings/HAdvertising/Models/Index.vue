...
const getSearch = throttle(() => {
    Inertia.get(route("admin.model"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        ...
        <TableCustom add="admin.model_add" class="mt-5" titlePage="MODELOS">
            ...
            <template #tbody>
                <Tbody v-for="(item, index) in marketings.data" :key="index">
                    ...
                    <td
                        class="flex  p-3"
                    >
                        <Link :href="route('admin.model_edit', item.id)">
                            <Icons
                                type="edit"
