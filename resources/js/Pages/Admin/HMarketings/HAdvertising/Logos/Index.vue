...
const getSearch = throttle(() => {
    Inertia.get(route("admin.logo"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        ...
        <TableCustom add="admin.logo_add" class="mt-5" titlePage="LOGOS DE MARCA">
            ...
            <template #tbody>
                <Tbody v-for="(item, index) in marketings.data" :key="index">
                    ...
                    <td
                        class="flex  p-3"
                    >
                        <Link :href="route('admin.logo_edit', item.id)">
                            <Icons
                                type="edit"
