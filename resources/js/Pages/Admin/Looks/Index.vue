...
const confirmDelete = async (id) => {
    ...
    if (result.isConfirmed) {
        Inertia.delete(route("admin.look_destroy", id), {
            onError: (errors) => {
                console.log(errors)
                Swal.fire({
                    icon: "error",
...
const getSearch = throttle(() => {
    Inertia.get(route("admin.look"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        <TableCustom add="admin.look_add" class="mt-5">
            <template #search>
                <div class="pt-2 relative mx-auto text-gray-600">
                    <input
                        ...
                        name="search"
                        v-model="q"
                        placeholder="Buscar: look & trends"
                    />
                    <button
                        type="submit"
            ...
            <template #thead>
                <TheadTable>Look & Trend</TheadTable>
                <TheadTable>Acciones</TheadTable>
            </template>
            <template #tbody>
                <Tbody v-for="(item, index) in looks.data" :key="index">
                    ...
                    >
                        <Link :href="route('admin.look_edit', item.id)">
                            <Icons
                                type="edit"
