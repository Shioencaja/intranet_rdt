...
const confirmDelete = async (id) => {
    ...
    if (result.isConfirmed) {
        Inertia.delete(route("admin.event_destroy", id), {
            onError: (errors) => {
                console.log(errors)
                Swal.fire({
                    icon: "error",
...
const getSearch = throttle(() => {
    Inertia.get(route("admin.event"), { q: q.value }, { preserveState: true });
}, 250);
...
<template>
    <LayoutAdmin>
        <TableCustom add="admin.event_add" class="mt-5">
            <template #search>
            ...
            <template #thead>
                <TheadTable>Evento</TheadTable>
                <TheadTable>Fecha y hora</TheadTable>

                <TheadTable>Acciones</TheadTable>
            </template>
            <template #tbody>
                <Tbody v-for="(item, index) in events.data" :key="index">
                    ...
                    >
                        <Link :href="route('admin.event_edit', item.id)">
                            <Icons
                                type="edit"
